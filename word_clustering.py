...
if __name__ == '__main__':
    ...
    filename = path = 'data/cache/{}'.format(
        get_cache_filename_from_args(args))
    cluster_to_words = None
    start_time = time.time()

    vector_file = args.glove_path + '/' + \
        'glove.6B.' + str(args.vector_dim) + 'd.txt'
    # df, labels_array = build_word_vector_matrix(vector_file, args.num_words)
    df, labels_array = custom_build_word_vector_matrix(vector_file)
    ...
    if not os.path.isfile(filename):
        ...
        kmeans_model = KMeans(
            init='k-means++', n_clusters=args.num_clusters, n_jobs=args.n_jobs, n_init=10)
        kmeans_model.fit(df)

        cluster_labels = kmeans_model.labels_
        # cluster_inertia = kmeans_model.inertia_
        cluster_to_words = list(find_word_clusters(
            labels_array, cluster_labels).values())

        # cache these clustering results
        save_json(path, cluster_to_words)
        print('Saved {} clusters to {}. Cached for later use.'.format(
            len(cluster_to_words), path))
